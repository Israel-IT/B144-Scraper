...
| D29 | *(Superseded by D35.)* **A 403/429 pauses every worker** for the backoff time (`Client._pause_all`), not just the one that got it | With 100 workers, per-thread backoff alone would keep hitting the site while it is asking us to slow down. |
...
| D32 | **Every Excel says which categories it covers.** The file name is `b144_all-categories_…`, `b144_<category>_…` or `b144_<n>-categories_…`. Run info gets a "Categories" line. The page shows "Run #N · <scope>" above each Download button, and a warning when the run wasn't all categories. The "Unfinished run" notice names the run's scope and says that Resume keeps it (the sidebar doesn't change it). | User report: "all the companies are from only one category, but we chose all categories." It wasn't a scraping bug. Run #7 had been started from the CLI with only עורכי דין, and it was resumed from the page while the sidebar showed "All categories" on. No all-categories run had been started yet. |
| D33 | **Stop / pause shutdown order: category threads first, then the worker pool, without `cancel_futures`.** | User report: "started getting errors, then I stopped and it froze." Python's `ThreadPoolExecutor.shutdown(cancel_futures=True)` marks queued futures cancelled but never wakes threads waiting on them in `as_completed`/`wait` (only a worker dequeuing the item does that). The category threads (D28) wait on exactly those futures, so they, and then the runner, waited forever. Reproduced offline: a paused run's runner thread was still alive after 45 s. With the new order it exits in 9–12 s. |
| D34 | **`DetailQueue._finished` skips cancelled futures before taking its lock** | A second freeze: shutdown held the pool's lock while cancelling and ran this callback, which wanted the queue's lock, while `add()` held the queue's lock and waited for the pool's lock in `submit()`. The old code deadlocked on its first try in a stress test; the fixed code passed 5 of 5. |
| D35 | **Adaptive speed (`http.Throttle`)**: a shared, first-come-first-served limit on requests in flight. It starts at 20 and adds 1 per 50 successes, up to the slider value. On push-back it halves and every worker pauses together, 15 s at first and doubling up to 5 min while the block lasts. Push-back means HTTP 403/429, connection reset/refused (curl 7/35/52/55/56) or a bot-challenge page. Above the last push-back level it climbs 10× slower. Push-back doesn't use up a request's retries. After 30 min without a single success, the run pauses with a message. This replaces D29. | Run #9 at 100 concurrent requests, 0.3–0.5 s delay: after ~3½ min B144's firewall reset 1,177 connections and then blocked this IP for ~12 minutes. Per-request retries kept hammering it and marked categories as errors. |
| D36 | **A bot-challenge page is push-back, not an immediate pause.** Challenges back off and retry with fresh cookies. StealthyFetcher is tried every third time, and only while its browser is installed; once it's found missing it isn't tried again. | The first live test after the block got one challenge page at ~30 concurrent requests. Because StealthyFetcher's browser isn't installed, the whole run paused (and then froze, D33). |
| D37 | **The page shows "Stopping…"** while in-flight requests finish (up to 30 s, the curl timeout), and the throttle state ("the site is limiting us: paused N s, then X of Y concurrent requests"). | The UI used to look unchanged after pressing Stop, so the user pressed it again. |
...
| 100 (fixed pool, run #9) | — | Blocked by the site's firewall after ~3½ min (connection resets), then the IP was blocked for ~12 min |
| 100 (adaptive, D35) | ~19 detail pages/s plus listings | Climbed 20 → 93 in 3 min without push-back, 0 errors. Throughput stayed flat from ~40 upward: the site answers more slowly instead of faster, so above ~40 the only effect is more risk. |
